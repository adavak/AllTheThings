createLocalizationString({
	...
	text = {
		...
		-- TODO: ko = "",
		cn = "如果希望在小列表中显示时将所有相关数据嵌套到给定的 NPC（常见首领掉落、掉落等），请启用此选项。此选项可能会导致大量重复，但其想法是该 NPC 将小列表中保持可见，就好像需要特定于该 NPC 的内容一样。\n\n默认：关闭",
		tw = "如果希望在小列表中顯示時將所有相關資料嵌套到給定的 NPC（常見首領掉落、掉落等），請啟用此選項。此選項可能會導致大量重複，但其想法是該 NPC 將在小列表中保持可見，就好像需要特定於該 NPC 的內容一樣。\n\n預設：關閉",
	},
})
